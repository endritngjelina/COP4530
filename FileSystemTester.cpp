...
private:
    ...
    void logTest(const std::string& testName, bool passed, int points) {
        ...
        else testOutput << "✗ " << testName << " [+0/" << points << " points]\n";
    ...
    bool testMkdir(FileSystem& fs, int points = 10) {
        ...
        try {
            ...
            if (result.find("test_dir/") == std::string::npos) success = false;
            

            fs.cd("test_dir");
            std::string pwd_result = fs.pwd();
            if (pwd_result != "/test_dir/") success = false;
            ...
            fs.cd("child");
            pwd_result = fs.pwd();
            if (pwd_result != "/parent/child/") success = false;
    ...
    bool testTouch(FileSystem& fs, int points = 10) {
        ...
        try {
            fs.touch("test.txt");
            std::string result = fs.ls();
            if (result.find("test.txt") == std::string::npos) success = false;
